...
初步实验结果表明，EfficientFormerV2-S0 能够在较短训练流程下学习到有效的遥感图像特征。当前 100 batch 基线模型在验证集 480 个样本上达到 0.7729 准确率，在测试集 480 个样本上达到 0.7521 准确率。增强训练策略在短训练条件下未超过基线准确率，但获得了更低的验证损失，说明其可能改善模型输出分布和泛化潜力。本文最后对系统实现、实验结果、类别混淆现象和后续优化方向进行了总结。
...
Preliminary results show that EfficientFormerV2-S0 can learn effective remote sensing image features within a short training budget. The current 100-batch baseline model achieves 0.7729 validation accuracy on 480 validation samples and 0.7521 test accuracy on 480 test samples. The enhanced setting does not outperform the baseline in short training, but it obtains a lower validation loss, suggesting potential improvements in output calibration and generalization under longer training. The project provides a complete, reproducible implementation basis for further experiments and report writing.
...
| 基线测试集混淆矩阵 | `outputs/figures/baseline_100b_test_confusion_matrix.png` |
| 基线测试集每类准确率 | `outputs/figures/baseline_100b_test_per_class_accuracy.png` |
...
当前最好模型 `baseline_100b_best.pt` 在测试集 480 个样本上达到 0.7521 准确率，与验证集 0.7729 接近。该结果说明模型在初步测试中具有一定泛化能力，没有只在验证集上出现孤立表现。
...
从当前结果看，模型对 `Residential`、`HerbaceousVegetation` 和 `AnnualCrop` 等类别表现较好，对 `River`、`Pasture` 和 `Forest` 等类别表现相对较弱。可能原因包括：
...
本文完成了一个基于 EfficientFormerV2 与增强训练策略的 EuroSAT 遥感土地利用图像分类系统。系统覆盖数据加载、模型训练、增强策略、独立评估、实验汇总、图表生成、命令行预测和 Web 演示等完整流程。当前实验结果表明，EfficientFormerV2-S0 在短程训练下已经能够取得较好的分类效果，测试集初步准确率达到 0.7521。
