...
实验结果表明，EfficientFormerV2-S0 能够在有限训练轮次下学习到有效的遥感图像特征。完成 5 epoch 训练后，基线模型在验证集 4050 个样本上达到 0.9785 准确率，在完整测试集 4050 个样本上达到 0.9763 准确率。增强训练策略在当前设置下未超过基线准确率，但 3 epoch 增强模型仍达到 0.9430 验证准确率，为后续优化 Mixup 强度、学习率调度和训练轮数提供了对比依据。本文最后对系统实现、实验结果、类别混淆现象和后续优化方向进行了总结。
...
- 对比 baseline 与 enhanced 配置在不同训练规模下的实验结果。
...
本章从遥感土地利用分类的应用背景出发，说明了采用轻量化深度学习模型进行 EuroSAT 图像分类的研究意义，并概括了本文在数据处理、模型训练、增强策略对比、实验分析和系统演示方面的主要工作。后续章节将围绕数据集、模型方法、系统实现和实验结果展开。
...
本章介绍了项目使用的 EuroSAT RGB 数据集、EfficientFormerV2-S0 主干模型、迁移学习思想和增强训练策略。EuroSAT 提供了具有代表性的 10 类土地利用图像，EfficientFormerV2-S0 则在参数量和表达能力之间取得较好平衡，为后续构建轻量化遥感图像分类系统提供了基础。
...
本章从工程实现角度说明了系统的总体架构和关键模块。项目将数据处理、模型构建、训练验证、评估分析、图表生成和交互式预测拆分为相对独立的模块，既便于实验复现和结果追踪，也便于后续替换模型、调整增强策略或扩展 Web 演示功能。
...
实验过程采用由小到大的推进方式：首先通过小规模训练验证数据、模型、训练和评估链路，然后扩展到完整训练集与验证集上的 1 epoch、3 epoch 和 5 epoch 实验。最终主结果采用 `baseline_5epoch`，用于报告中的核心性能分析。

本文主要使用准确率和交叉熵损失评价模型表现。准确率反映预测类别与真实类别一致的样本比例，交叉熵损失用于衡量模型输出概率分布与真实标签之间的差异。除总体准确率外，本文还统计混淆矩阵和逐类准确率，以观察不同土地利用类别之间的混淆关系。
...
从实验结果可以看出，随着训练规模增加，基线模型准确率明显提升。`baseline_300b` 的验证准确率达到 0.9443，完整 1 epoch 的 `baseline_full_epoch` 达到 0.9560，3 epoch 的 `baseline_3epoch` 达到 0.9704，5 epoch 的 `baseline_5epoch` 进一步达到 0.9785。这说明 EfficientFormerV2-S0 能够有效学习 EuroSAT 图像中的遥感场景特征，也说明项目的数据处理、模型训练和评估流程是有效的。

当前最好模型在完整测试集 4050 个样本上达到 0.9763 准确率，与验证集 0.9785 接近。该结果说明模型在测试集上具有较好的泛化能力，没有只在验证集上出现孤立表现。

### 4.5 增强策略分析

在 300 batch、完整 1 epoch 和 3 epoch 设置下，增强配置的验证准确率均低于基线配置。3 epoch 设置中，`enhanced_3epoch` 的验证准确率为 0.9430，低于 `baseline_3epoch` 的 0.9704。考虑到 5 epoch baseline 已达到 0.9763 测试准确率，本文将 `baseline_5epoch` 作为最终主模型。该结果说明当前 Mixup、ColorJitter 和 Label Smoothing 组合在本实验设置下尚未带来更高准确率，后续可以降低 Mixup 强度或加入学习率调度再验证。

这一现象说明，在当前增强强度和训练预算下，Mixup 和 Label Smoothing 可能增加分类边界学习难度，使准确率低于基线；但增强策略在部分实验中表现出较低的损失，说明其可能改善模型输出分布或置信度校准。后续如果进一步调节增强强度、学习率调度和训练轮数，增强策略仍有可能带来更稳定的泛化效果。
...
- `Forest` 与 `HerbaceousVegetation` 在局部植被纹理上可能存在相似区域，导致少量样本被误判。
...
本章给出了实验环境、训练配置、代表性结果和类别表现分析。实验表明，随着训练轮数增加，基线模型在 EuroSAT 数据集上的验证和测试准确率持续提升，5 epoch 基线模型在完整测试集上达到 0.9763 准确率。增强策略在当前参数设置下未超过基线，但提供了有价值的对照结果。类别分析表明，模型主要混淆集中在植被、农田和建筑相关类别之间。
...
本章介绍了基于 Streamlit 的 Web 演示系统。该界面将训练得到的模型封装为可交互应用，使用户能够通过上传遥感图像查看 Top-k 分类结果。演示系统提升了项目的展示性，也为后续扩展批量预测、多模型对比和可解释性可视化提供了接口基础。
...
[2] Li Y, Hu J, Wen Y, et al. Rethinking vision transformers for MobileNet size and speed[C]//Proceedings of the IEEE/CVF International Conference on Computer Vision. 2023.

[3] Wightman R. PyTorch Image Models[EB/OL]. https://github.com/huggingface/pytorch-image-models.

[4] PyTorch Contributors. PyTorch Documentation[EB/OL]. https://pytorch.org/docs/.
