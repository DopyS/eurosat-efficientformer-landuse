...
初步实验结果表明，EfficientFormerV2-S0 能够在较短训练流程下学习到有效的遥感图像特征。完成 5 epoch 训练后，基线模型在验证集 4050 个样本上达到 0.9785 准确率，在完整测试集 4050 个样本上达到 0.9763 准确率。增强训练策略在当前设置下未超过基线准确率，但 3 epoch 增强模型仍达到 0.9430 验证准确率，为后续优化 Mixup 强度、学习率调度和训练轮数提供了对比依据。本文最后对系统实现、实验结果、类别混淆现象和后续优化方向进行了总结。
...
```bash
python -m src.eurosat_landuse.train --config configs/baseline.yaml --download --epochs 5 --batch-size 32 --run-name baseline_5epoch
```
...
```bash
python -m src.eurosat_landuse.evaluate --config configs/baseline.yaml --checkpoint outputs/checkpoints/baseline_5epoch_best.pt --split test --batch-size 32 --run-name baseline_5epoch_eval_test_full
```
...
| 5 epoch 基线测试集混淆矩阵 | `outputs/figures/baseline_5epoch_test_full_confusion_matrix.png` |
| 5 epoch 基线测试集每类准确率 | `outputs/figures/baseline_5epoch_test_full_per_class_accuracy.png` |
...
为进一步观察错误样本，项目基于当前最好模型 `baseline_5epoch_best.pt` 导出了 `Forest -> HerbaceousVegetation`、`PermanentCrop -> HerbaceousVegetation` 和 `Industrial -> Residential` 三组典型误分类图像，并分别生成拼图。相关路径包括 `outputs/error_samples/baseline_5epoch_test_Forest_to_HerbaceousVegetation/contact_sheet.png`、`outputs/error_samples/baseline_5epoch_test_PermanentCrop_to_HerbaceousVegetation/contact_sheet.png` 和 `outputs/error_samples/baseline_5epoch_test_Industrial_to_Residential/contact_sheet.png`。从报告写作角度，这些图可用于展示模型在植被覆盖、农田纹理和建筑区域场景中的典型混淆情况。
...
本文完成了一个基于 EfficientFormerV2 与增强训练策略的 EuroSAT 遥感土地利用图像分类系统。系统覆盖数据加载、模型训练、增强策略、独立评估、实验汇总、图表生成、命令行预测和 Web 演示等完整流程。当前实验结果表明，EfficientFormerV2-S0 在 5 epoch 训练后已经能够取得较好的分类效果，完整测试集准确率达到 0.9763。
