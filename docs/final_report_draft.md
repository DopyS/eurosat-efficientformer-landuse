...
初步实验结果表明，EfficientFormerV2-S0 能够在较短训练流程下学习到有效的遥感图像特征。将训练规模扩大到 300 batch 后，基线模型在验证集 1920 个样本上达到 0.9443 准确率，在完整测试集 4050 个样本上达到 0.9356 准确率。增强训练策略在当前设置下未超过基线准确率，但为后续优化 Mixup 强度、学习率调度和训练轮数提供了对比依据。本文最后对系统实现、实验结果、类别混淆现象和后续优化方向进行了总结。
...
Preliminary results show that EfficientFormerV2-S0 can learn effective remote sensing image features within a short training budget. After increasing the training scale to 300 batches, the baseline model achieves 0.9443 validation accuracy on 1920 validation samples and 0.9356 accuracy on the full test split with 4050 samples. The enhanced setting does not outperform the baseline under the current setup, but it provides a useful comparison for further tuning Mixup strength, learning rate scheduling and training duration. The project provides a complete, reproducible implementation basis for further experiments and report writing.
...
```bash
python -m src.eurosat_landuse.train --config configs/baseline.yaml --download --epochs 1 --batch-size 32 --max-train-batches 300 --max-val-batches 60 --run-name baseline_300b
```
...
```bash
python -m src.eurosat_landuse.evaluate --config configs/baseline.yaml --checkpoint outputs/checkpoints/baseline_300b_best.pt --split test --batch-size 32 --run-name baseline_300b_eval_test_full
```
...
| 300 batch 基线完整测试集混淆矩阵 | `outputs/figures/baseline_300b_test_full_confusion_matrix.png` |
| 300 batch 基线完整测试集每类准确率 | `outputs/figures/baseline_300b_test_full_per_class_accuracy.png` |
| Web 演示界面截图 | `outputs/figures/streamlit_demo_ui.png` |

### 4.4 基线实验分析

从实验结果可以看出，随着训练 batch 数增加，基线模型准确率明显提升。`quick_baseline` 的验证准确率只有 0.0625，`baseline_100b` 达到 0.7729，而 `baseline_300b` 进一步提升到 0.9443。这说明 EfficientFormerV2-S0 能够较快学习到 EuroSAT 图像中的有效特征，也说明项目的数据处理、模型训练和评估流程是有效的。

当前最好模型 `baseline_300b_best.pt` 在完整测试集 4050 个样本上达到 0.9356 准确率，与验证集 0.9443 接近。该结果说明模型在测试集上具有较好的泛化能力，没有只在验证集上出现孤立表现。

### 4.5 增强策略分析

在 20 batch、100 batch 和 300 batch 设置下，增强配置的验证准确率均低于基线配置。300 batch 设置中，`enhanced_300b` 的验证准确率为 0.8620，低于 `baseline_300b` 的 0.9443。该结果说明当前 Mixup、ColorJitter 和 Label Smoothing 组合在短程训练下尚未带来更高准确率，后续需要进一步调参或延长训练。
...
从 300 batch 完整测试集结果看，模型整体表现明显提升，最弱类别 `Highway` 的准确率也达到 0.8760。相对薄弱类别包括 `Highway`、`Pasture`、`PermanentCrop`、`HerbaceousVegetation` 和 `AnnualCrop`。可能原因包括：
...
进一步统计混淆矩阵可知，300 batch 完整测试集中最明显的错误方向为 `PermanentCrop -> HerbaceousVegetation` 29 个、`Highway -> River` 17 个、`Pasture -> Forest` 14 个、`Residential -> Industrial` 14 个、`AnnualCrop -> PermanentCrop` 13 个。这些混淆方向说明，模型在处理线状地物、居住/工业纹理、植被覆盖和农田纹理等场景时仍存在改进空间。

为进一步观察错误样本，项目基于当前最好模型 `baseline_300b_best.pt` 导出了 `PermanentCrop -> HerbaceousVegetation`、`Highway -> River` 和 `Pasture -> Forest` 三组典型误分类图像，并分别生成拼图。相关路径包括 `outputs/error_samples/baseline_300b_test_PermanentCrop_to_HerbaceousVegetation/contact_sheet.png`、`outputs/error_samples/baseline_300b_test_Highway_to_River/contact_sheet.png` 和 `outputs/error_samples/baseline_300b_test_Pasture_to_Forest/contact_sheet.png`。从报告写作角度，这些图可用于展示模型在线状地物、植被覆盖和农田纹理等场景中的典型混淆情况。
...
本文完成了一个基于 EfficientFormerV2 与增强训练策略的 EuroSAT 遥感土地利用图像分类系统。系统覆盖数据加载、模型训练、增强策略、独立评估、实验汇总、图表生成、命令行预测和 Web 演示等完整流程。当前实验结果表明，EfficientFormerV2-S0 在 300 batch 短程训练下已经能够取得较好的分类效果，完整测试集准确率达到 0.9356。
