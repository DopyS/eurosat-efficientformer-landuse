...
为进一步观察错误样本，项目导出了 `River -> Highway`、`Forest -> SeaLake`、`PermanentCrop -> HerbaceousVegetation` 和 `Pasture -> Forest` 四组典型误分类图像，并分别生成拼图。相关路径包括 `outputs/error_samples/test_River_to_Highway/contact_sheet.png`、`outputs/error_samples/test_Forest_to_SeaLake/contact_sheet.png`、`outputs/error_samples/test_PermanentCrop_to_HerbaceousVegetation/contact_sheet.png` 和 `outputs/error_samples/test_Pasture_to_Forest/contact_sheet.png`。从报告写作角度，这些图可用于展示模型在线状地物、水体边界、植被覆盖和农田纹理等场景中的典型混淆情况。
