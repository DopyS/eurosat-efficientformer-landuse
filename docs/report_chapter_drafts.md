...
从实验结果可以看出，随着训练 batch 数增加，模型准确率显著提升。`quick_baseline` 的验证准确率仅为 0.0625，`baseline_100b` 的验证准确率达到 0.7729，`baseline_300b` 进一步提升到 0.9443，说明 EfficientFormerV2-S0 能够有效学习 EuroSAT 图像特征。

在 20 batch、100 batch 和 300 batch 设置下，增强配置的验证准确率均低于基线配置。300 batch 设置中，`enhanced_300b` 的验证准确率为 0.8620，低于 `baseline_300b` 的 0.9443。这说明 Mixup 和 Label Smoothing 在当前短程训练条件下可能增加优化难度，后续仍需要通过更长训练和参数调整验证增强策略价值。

当前最好模型 `baseline_300b_best.pt` 在完整测试集 4050 个样本上达到 0.9356 准确率，与验证集 0.9443 接近，说明模型具有较好的泛化能力。从类别表现看，`Highway`、`Pasture`、`PermanentCrop`、`HerbaceousVegetation` 和 `AnnualCrop` 是测试集上相对较弱的类别，后续可以结合混淆矩阵进一步分析其与相似纹理类别之间的混淆关系。
...
| 图 6 | 300 batch 基线模型完整测试集混淆矩阵 | `outputs/figures/baseline_300b_test_full_confusion_matrix.png` |
