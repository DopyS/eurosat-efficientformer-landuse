...
当前最好模型 `baseline_100b_best.pt` 在测试集 480 个样本上达到 0.7521 准确率，与验证集 0.7729 接近，说明模型在初步测试中具有一定泛化能力。从类别表现看，`River`、`Pasture` 和 `Forest` 是测试集上相对较弱的类别，后续可以结合混淆矩阵进一步分析其与相似纹理类别之间的混淆关系。
...
| 图 6 | 基线模型测试集混淆矩阵 | `outputs/figures/baseline_100b_test_confusion_matrix.png` |
