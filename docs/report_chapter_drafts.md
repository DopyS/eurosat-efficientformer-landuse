...
从实验结果可以看出，随着训练规模增加，模型准确率显著提升。`quick_baseline` 的验证准确率仅为 0.0625，`baseline_100b` 达到 0.7729，`baseline_300b` 提升到 0.9443，完整 1 epoch 的 `baseline_full_epoch` 达到 0.9560，3 epoch 的 `baseline_3epoch` 进一步达到 0.9704，说明 EfficientFormerV2-S0 能够有效学习 EuroSAT 图像特征。

在 20 batch、100 batch、300 batch、完整 1 epoch 和 3 epoch 设置下，增强配置的验证准确率均低于基线配置。3 epoch 设置中，`enhanced_3epoch` 的验证准确率为 0.9430，低于 `baseline_3epoch` 的 0.9704。后续仍需要通过更长训练和参数调整验证增强策略价值。

当前最好模型 `baseline_3epoch_best.pt` 在完整测试集 4050 个样本上达到 0.9664 准确率，与验证集 0.9704 接近，说明模型具有较好的泛化能力。从类别表现看，`HerbaceousVegetation`、`PermanentCrop`、`Industrial`、`AnnualCrop` 和 `Pasture` 是测试集上相对较弱的类别，后续可以结合混淆矩阵进一步分析其与相似纹理类别之间的混淆关系。
...
| 图 6 | 3 epoch 基线模型测试集混淆矩阵 | `outputs/figures/baseline_3epoch_test_full_confusion_matrix.png` |
