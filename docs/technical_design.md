...
7. 根据评估 JSON 生成错误分析 Markdown，整理弱项类别和主要混淆方向。
8. 根据指定混淆方向导出典型误分类样本和拼图，用于报告插图。
9. 将训练产物保存到本地 `outputs/`，但不提交到 GitHub。
10. 使用独立评估脚本加载 checkpoint，在验证集或测试集上生成整体指标和逐类指标。
...
当前增强配置文件为 `configs/enhanced.yaml`，基线配置文件为 `configs/baseline.yaml`。增强配置启用 ColorJitter、Mixup 和 Label Smoothing；基线配置关闭 Mixup 和 ColorJitter，用于形成同规模对比。
